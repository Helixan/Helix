...
public final class StringUtils {
    ...
    public static int editDistance(final String first, final HString second) {
        Validate.notNull(first, "No edit distance can be calculated on a null String");
        Validate.notNull(second, "No edit distance can be calculated on a null HString");
        return editDistance(first, second.toString());
    }

    public static int editDistance(final HString first, final String second) {
        Validate.notNull(first, "No edit distance can be calculated on a null HString");
        Validate.notNull(second, "No edit distance can be calculated on a null String");
        return editDistance(first.toString(), second);
    }

    public static int editDistance(final HString first, final HString second) {
        Validate.notNull(first, "No edit distance can be calculated on a null HString");
        Validate.notNull(second, "No edit distance can be calculated on a null HString");
    ...
    public static int editDistanceIgnoreCase(final String first, final HString second) {
        Validate.notNull(first, "No edit distance can be calculated on a null String");
        Validate.notNull(second, "No edit distance can be calculated on a null HString");
        return editDistance(first.toLowerCase(), second.toString().toLowerCase());
    }

    public static int editDistanceIgnoreCase(final HString first, final String second) {
        Validate.notNull(first, "No edit distance can be calculated on a null HString");
        Validate.notNull(second, "No edit distance can be calculated on a null String");
        return editDistance(first.toString().toLowerCase(), second.toLowerCase());
    }

    public static int editDistanceIgnoreCase(final HString first, final HString second) {
        Validate.notNull(first, "No edit distance can be calculated on a null HString");
        Validate.notNull(second, "No edit distance can be calculated on a null HString");
